import { createServerClient } from './supabase';

const LIMITS = {
  free:             3,    // per day
  'basic-monthly':  30,   // per month
  'basic-yearly':   30,
  basic:            30,
  'pro-monthly':    100,
  'pro-yearly':     100,
  pro:              100,
  team:             9999,
};

/**
 * Check if user is allowed to run a separation, and increment counter.
 * Uses a Postgres function (002_atomic_usage.sql) that takes a row-level lock,
 * eliminating the read-then-write race condition.
 * Returns { allowed, used, limit, plan, userId }
 */
export async function checkAndIncrementUsage(clerkUserId) {
  const db = createServerClient();

  const { data, error } = await db.rpc('check_and_increment_usage', {
    p_clerk_id: clerkUserId,
  });

  if (error) throw new Error(`Usage check failed: ${error.message}`);

  // rpc returns an array of rows from the RETURNS TABLE function
  const row = Array.isArray(data) ? data[0] : data;
  if (!row) throw new Error('Usage check returned no data');

  return {
    allowed: row.allowed,
    used: row.used,
    limit: row.lim,
    plan: row.user_plan,
    userId: row.user_id,
  };
}

/**
 * Returns true if the current time is past the start of a new billing cycle,
 * meaning the counter should be treated as 0. Mirrors the SQL RPC logic exactly.
 */
function isNewCycle(plan, resetAt) {
  const now = new Date();
  const last = resetAt ? new Date(resetAt) : new Date(0);
  if (!plan || plan === 'free') {
    // Daily reset: new cycle if today (UTC) > last reset day (UTC)
    const todayUTC = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate());
    const lastUTC  = Date.UTC(last.getUTCFullYear(), last.getUTCMonth(), last.getUTCDate());
    return todayUTC > lastUTC;
  }
  // Monthly reset: new cycle if this month (UTC) > last reset month (UTC)
  const thisMonth = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1);
  const lastMonth = Date.UTC(last.getUTCFullYear(), last.getUTCMonth(), 1);
  return thisMonth > lastMonth;
}

/**
 * Get current usage for a user without incrementing
 */
export async function getUsage(clerkUserId) {
  const db = createServerClient();
  const { data: user } = await db
    .from('users')
    .select('plan, credits_used_today, credits_reset_at')
    .eq('clerk_id', clerkUserId)
    .single();

  if (!user) return { used: 0, limit: LIMITS.free, plan: 'free' };

  const plan = user.plan || 'free';
  const limit = LIMITS[plan] || LIMITS.free;
  const used = isNewCycle(plan, user.credits_reset_at) ? 0 : (user.credits_used_today || 0);

  return { used, limit, plan };
}
